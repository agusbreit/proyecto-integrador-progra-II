var productos = [
   {
      nombre: 'Lenguetazos',
      descripcion: 'Caramelos lenguetazos en tres sabores: pinta lenguas, tutifruti y fiesta.',
      image: '/images/products/lenguetazo.png',
      fecha: 26 / 06,
   },
   {
      nombre: 'Chicle Beldent',
      descripcion: 'Chicles Beldent sabor menta. 8 unidades, frecura garantizada.',
      image: '/images/products/beldent_menta.png',
      fecha: 10 / 05,
   },
   {
      nombre: 'Sugus confitados',
      descripcion: 'Sugus confitados sabor frutilla, moras y limon',
      image: '/images/products/sugus-confitados-1.jpeg',
      fecha: 25 / 02,
   },
   {
      nombre: 'Alfajor Milka Oreo',
      descripcion: 'Alfajor torta sabor Oreo y con el mejor chocolate Milka, delicioso.',
      image: '/images/products/Milka_oreo.jpeg',
      fecha: 30 / 10,
   },
   {
      nombre: 'Chocolate Arcor',
      descripcion: 'Chocolate con leche y mani, apto para celiacos',
      image: '/images/products/chocoarcor.jpeg',
      fecha: 16 / 11,
   },
   {
      nombre: 'Chocolate Block',
      descripcion: 'Chocolate con leche y mani marca Cofler. 38 g la unidad, irresistible.',
      image: '/images/products/block.jpeg',
      fecha: 05 / 05,
   },
   {
      nombre: 'Cerebritos Mogul',
      descripcion: 'Gomitas rellenas en forma de cerebrito marcra Mogul. Sabores limon, fresa, banana y manzana.',
      image: '/images/products/mogul.jpeg',
      fecha: 20 / 03,
   },
   {
      nombre: 'Caramelos rellenos',
      descripcion: 'Caramelos rellenos frutales Arcor. Sabor cereza, banana, manzana, durazno y anana.',
      image: '/images/products/arcorrellenos.jpeg',
      fecha: 22 / 04,
   },
   {
      nombre: 'Rocklets',
      descripcion: 'Paquetes de rocklets de todos los colores. Rellenos de mani o chocolate',
      image: '/images/products/rockletsing.jpeg',
      fecha: 10 / 05,
   },
   {
      nombre: 'Chupetin',
      descripcion: 'Chupetines sabor frutilla, limon, manzana y naranja',
      image: '/images/products/chupetinok.jpeg',
      fecha: 26 / 09,
   },
   {
      nombre: 'Bananita Dolca',
      descripcion: 'Golosina sabor banana cubierta en chocolate con leche',
      image: '/images/products/bananitaimg.jpeg',
      fecha: 21 / 07,
   },
   {
      nombre: 'Tita',
      descripcion: 'Galleta rellena de limon, cubierta en chocolate semiamargo',
      image: '/images/products/Tita.jpeg',
      fecha: 08 / 01,
   },
]
module.exports = productos;